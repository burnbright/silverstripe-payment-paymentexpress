 ...
 * 
 */
class PaymentExpressHostedPayment extends Payment{
	...
	*/
	 ...
	 */
	static function createPxPay(){
		if(!self::$pxpay){
			$protocol = (strtolower(self::$protocol) == "openssl") ? "PxPay_OpenSSL" : "PxPay_Curl";
			self::$pxpay = new $protocol(self::$pxPay_Url, self::$pxPay_Userid, self::$pxPay_Key);
		}
		return self::$pxpay;
	}
	...
	public function processPayment($data, $form){
		...
		if(self::$use_iframe){
			return new Payment_Processing(array(
				'Content' => "<iframe src =\"$url\" width=\"100%\" height=\"380\" frameborder=\"0\" name=\"payframe\"><a href=\"$url\">"._t('PaymentExpressHostedPayment.CLICKHERE',"click here to pay")."</a></iframe>"
			));
		}
	...
	protected function prepareRequest($data){
		...
		$postProcess_url = Director::absoluteBaseURL() .PaymentExpressHostedPayment_Controller::$URLSegment."/processResponse";
...
class PaymentExpressHostedPayment_Controller extends Controller {
	...
	public function processResponse() {
		...
		$pxpay = PaymentExpressHostedPayment::createPxPay();
		...
		if(isset($dpsDirectlyConnecting) && $dpsDirectlyConnecting) {
			...
			$payment = DataObject::get_one('PaymentExpressHostedPayment', "`TxnID` = '$SQL_paymentID'");
			if(!$payment) {
